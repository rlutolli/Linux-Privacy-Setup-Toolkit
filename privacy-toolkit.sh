...
install_bitwarden_from_github() {
    ...
    # Get latest version and construct URL
...
install_security_tools() {
    ...
    case $DISTRO in
        ubuntu|debian)
            ...
            # Bug 1 Fix: bitwarden-cli is not in official repos, use fallback mechanism
        ...
        fedora)
            ...
            # Bug 2 Fix: Install CLI tool, not the desktop app
            # Bitwarden CLI is not in Fedora repos, use GitHub binary
            if ! command -v bw &> /dev/null; then
                print_color "$CYAN" "  Installing Bitwarden CLI from GitHub..."
                install_bitwarden_from_github || true
            else
                print_color "$YELLOW" "  ⏭ Bitwarden CLI already installed, skipping"
            fi
        ...
        arch|manjaro)
            ...
            # Bitwarden CLI via AUR or GitHub fallback
            if ! command -v bw &> /dev/null; then
                if command -v yay &> /dev/null; then
                    print_color "$CYAN" "  Installing Bitwarden CLI via AUR (yay)..."
                    if yay -S --noconfirm bitwarden-cli 2>/dev/null; then
                        print_color "$GREEN" "  ✓ Bitwarden CLI installed via AUR"
                        log_message "Installed: Bitwarden CLI (AUR)"
                    else
                        print_color "$YELLOW" "  ⏭ AUR installation failed, trying GitHub binary..."
                        install_bitwarden_from_github || true
                    fi
                elif command -v paru &> /dev/null; then
                    print_color "$CYAN" "  Installing Bitwarden CLI via AUR (paru)..."
                    if paru -S --noconfirm bitwarden-cli 2>/dev/null; then
                        print_color "$GREEN" "  ✓ Bitwarden CLI installed via AUR"
                        log_message "Installed: Bitwarden CLI (AUR)"
                    else
                        print_color "$YELLOW" "  ⏭ AUR installation failed, trying GitHub binary..."
                    ...
                    print_color "$YELLOW" "  ⏭ AUR helper not available, using GitHub binary..."
                    install_bitwarden_from_github || true
                fi
            else
                print_color "$YELLOW" "  ⏭ Bitwarden CLI already installed, skipping"
